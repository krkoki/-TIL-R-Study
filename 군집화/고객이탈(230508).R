df <- read.csv('./data/customer/customer.csv')
head(df)
dim(df)
summary(df)
...
fviz_nbclust(df_scaled[,-17], FUN=kmeans, method = "silhouette")

#산점도 행렬(클러스터가 2개일 경우)
plot(df_scaled[,-17], pch=result[[2]]$cluster, col=result[[2]]$cluster)
